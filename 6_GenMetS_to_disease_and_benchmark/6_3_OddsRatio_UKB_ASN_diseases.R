...
myPRS19 <- fread("../../0_data/ukb_asn_prs.txt")
...
prslist=c("GenMetS", "Walree2022_MetS", "Lind2019_MetS", "T2DM_SAS", "T2DM_EUR", "HF_meta", "MI_EUR", "CAD_EUR", "Hypertension_EUR", "Stroke_EAS", "Stroke_EUR")


cat("UKB ASN women\n")
myUKB_ASN_women <-  mydata %>% dplyr::filter(sex=="Female")
dim(myUKB_ASN_women)
colidx <- which(colnames(myUKB_ASN_women) %in% prslist) 
Nrow <- length(colidx)
...
dim(myUKB_ASN_men)
colidx <- which(colnames(myUKB_ASN_men) %in% prslist) 
Nrow <- length(colidx)
cat("we are studying ", Nrow, " PGSs\n")
...
write.table(myresult_all[idx, ], "OR_benchmark_diseasePGS_in_UKB_ASN.txt", row.names=F, sep="\t", quote=F)
