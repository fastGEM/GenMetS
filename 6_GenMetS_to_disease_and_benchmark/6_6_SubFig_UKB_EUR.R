...
###############################UKB_EUR
UKB_EUR<- fread("../../0_data/OddsRatio_result/OR_benchmark_diseasePGS_in_UKB_EUR.txt") %>% data.frame() %>%
  dplyr::filter(!is.na(disease))
unique(UKB_EUR$predictor)
unique(UKB_EUR$cohort)

#rename predictor as AuthorYear_disease
idx<-which(UKB_EUR$predictor=="HF_meta"); UKB_EUR$predictor[idx]<- "Levin2022_HF"
idx<-which(UKB_EUR$predictor=="MI_EUR"); UKB_EUR$predictor[idx]<- "Hartiala2021_MI"
idx<-which(UKB_EUR$predictor=="T2DM_EUR"); UKB_EUR$predictor[idx]<- "Suzuki2024_T2DM"
idx<-which(UKB_EUR$predictor=="CAD_EUR"); UKB_EUR$predictor[idx]<- "Aragam2022_CAD"
idx<-which(UKB_EUR$predictor=="Hypertension_EUR"); UKB_EUR$predictor[idx]<- "Sinnott2021_HTN"
idx<-which(UKB_EUR$predictor=="Stroke_EUR"); UKB_EUR$predictor[idx]<- "Mishra2022_stroke"
unique(UKB_EUR$predictor)

UKB_EUR$pred_cohort<- paste0(UKB_EUR$predictor, ":", UKB_EUR$cohort)

#t2dm
unique(UKB_EUR$disease)
t2dm<- UKB_EUR %>% dplyr::filter(disease=="t2dm") %>%
...
levels=rev(c("GenMetS:UKB EUR women","GenMetS:UKB EUR men",
             "Walree2022_MetS:UKB EUR women", "Walree2022_MetS:UKB EUR men",
             "Lind2019_MetS:UKB EUR women", "Lind2019_MetS:UKB EUR men",
             "Suzuki2024_T2DM:UKB EUR women", "Suzuki2024_T2DM:UKB EUR men")))
...
################HF
HF<- UKB_EUR %>% dplyr::filter(disease=="HF") %>%
...
HF$pred_cohort <- factor(HF$pred_cohort,
  levels=rev(c("GenMetS:UKB EUR women","GenMetS:UKB EUR men",
               "Walree2022_MetS:UKB EUR women", "Walree2022_MetS:UKB EUR men",
               "Lind2019_MetS:UKB EUR women", "Lind2019_MetS:UKB EUR men",
               "Levin2022_HF:UKB EUR women", "Levin2022_HF:UKB EUR men")))
...
##########Hyertension
HTN<- UKB_EUR %>% dplyr::filter(disease=="hypertension") %>%
...
HTN$pred_cohort <- factor(HTN$pred_cohort,
                         levels=rev(c("GenMetS:UKB EUR women","GenMetS:UKB EUR men",
                                      "Walree2022_MetS:UKB EUR women", "Walree2022_MetS:UKB EUR men",
                                      "Lind2019_MetS:UKB EUR women", "Lind2019_MetS:UKB EUR men",
                                      "Sinnott2021_HTN:UKB EUR women", "Sinnott2021_HTN:UKB EUR men")))
...
##########4MI
MI<- UKB_EUR %>% dplyr::filter(disease=="MI") %>%
...
MI$pred_cohort <- factor(MI$pred_cohort,
                          levels=rev(c("GenMetS:UKB EUR women","GenMetS:UKB EUR men",
                                       "Walree2022_MetS:UKB EUR women", "Walree2022_MetS:UKB EUR men",
                                       "Lind2019_MetS:UKB EUR women", "Lind2019_MetS:UKB EUR men",
                                       "Hartiala2021_MI:UKB EUR women", "Hartiala2021_MI:UKB EUR men")))
...
##########5Stroke
stroke<- UKB_EUR %>% dplyr::filter(disease=="stroke") %>%
...
stroke$pred_cohort <- factor(stroke$pred_cohort,
                         levels=rev(c("GenMetS:UKB EUR women","GenMetS:UKB EUR men",
                                      "Walree2022_MetS:UKB EUR women", "Walree2022_MetS:UKB EUR men",
                                      "Lind2019_MetS:UKB EUR women", "Lind2019_MetS:UKB EUR men",
                                      "Mishra2022_stroke:UKB EUR women", "Mishra2022_stroke:UKB EUR men")))
...
##########6CAD
CAD<- UKB_EUR %>% dplyr::filter(disease=="CAD") %>%
...
CAD$pred_cohort <- factor(CAD$pred_cohort,
                             levels=rev(c("GenMetS:UKB EUR women","GenMetS:UKB EUR men",
                                          "Walree2022_MetS:UKB EUR women", "Walree2022_MetS:UKB EUR men",
                                          "Lind2019_MetS:UKB EUR women", "Lind2019_MetS:UKB EUR men",
                                          "Aragam2022_CAD:UKB EUR women", "Aragam2022_CAD:UKB EUR men")))
...
final_plot <- annotate_figure(combined_plot,
                              top = text_grob("UKB EUR", size = 14, face = "bold"))

# Print the final plot
print(final_plot)

ggsave("SubFig_Benchmark_UKB_EUR.tiff",  
       width =8, height =5, units = "in",dpi = 600, compression = "lzw")

ggsave("SubFig_Benchmark_UKB_EUR.pdf",  
