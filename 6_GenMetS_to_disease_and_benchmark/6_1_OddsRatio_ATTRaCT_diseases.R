...
#due to the different results
#from matchit, though we
#fix the seed. 
######################
...
diseases <- c("T2DM", "STROKE", "HF", "CAD", "MI", "HTN" )
...
prslist=c("GenMetS", "Walree2022_MetS", "Lind2019_MetS",  "T2DM_EAS", "T2DM_EUR", "HF_meta", "MI_EUR", "CAD_EUR", "Hypertension_EUR", "Stroke_EAS", "Stroke_EUR")

myraw <- mydata; #backup myraw
cat("OddsRatio for ATTRACT women\n")
myATTRACT_women <- myraw %>% dplyr::filter(GENDER=="Female")
dim(myATTRACT_women)
colidx <- which(colnames(myATTRACT_women) %in% prslist)
Nrow <- length(colidx)
...
cat("OddsRatio for ATTRaCT women is done \n")  
...
dim(myATTRACT_men)
colidx <- which(colnames(myATTRACT_men) %in% prslist)
Nrow <- length(colidx)
cat("we are studying ", Nrow, " PGSs\n")
...
write.table(myresult_all[idx,] , "../../0_data/OddsRatio_result/OR_benchmark_diseasePGS_in_ATTRACT.txt", row.names=F, sep="\t", quote=F)
