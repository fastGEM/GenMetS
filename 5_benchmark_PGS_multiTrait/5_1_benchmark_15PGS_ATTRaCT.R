...
myPRS20 <- fread("../../0_data/studysample_20PGS.txt")
...
dim(myATTRACT_women)
colidx <- which(colnames(myATTRACT_women) %in% pgslist$PGS)
Nrow <- length(colidx)
...
dim(myATTRACT_men)
colidx <- which(colnames(myATTRACT_men) %in% pgslist$PGS)
Nrow <- length(colidx)
cat("we are studying ", Nrow, " PGSs\n")
...
write.table(myresult_all, "../../0_data/OddsRatio_result/OddsRatio_diseases_ATTRaCT.txt", row.names=F, sep="\t", quote=F)
