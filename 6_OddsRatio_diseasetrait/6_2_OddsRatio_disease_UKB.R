...
if(!file.exists("UKB_Asianwomenmen_s8792_mets_diseases_others_updated.txt")){
mydata <- fread("UKB_Asianwomenmen_s8792_mets_diseases_others.txt") %>% data.frame()
myMI <- fread("data_UKB_MI.txt"); table(myMI$MI)
myMI$MI <- factor(myMI$MI, levels=c("control", "case"), labels=c(0, 1))
dim(mydata)
mydata <- merge(mydata, myMI, by.x="ID", by.y="eid", all.x=T)
dim(mydata)
write.table(mydata,"UKB_Asianwomenmen_s8792_mets_diseases_others_updated.txt",
            row.names=F, sep="\t", quote=F)
}

mydata <- fread("UKB_Asianwomenmen_s8792_mets_diseases_others_updated.txt") %>% data.frame()
...
cat("z-score for 20 PGSs\n")
pgs20<-fread("../PGS20.txt")

cat("UKB ASN women\n")
myUKB_ASN_women <-  mydata %>% dplyr::filter(sex=="Female")
dim(myUKB_ASN_women)
colidx <- which(colnames(myUKB_ASN_women) %in% pgs20$PGS20) 
Nrow <- length(colidx)
...
dim(myUKB_ASN_men)
colidx <- which(colnames(myUKB_ASN_men) %in% pgs20$PGS20) 
Nrow <- length(colidx)
cat("we are studying ", Nrow, " PGSs\n")
...
write.table(myresult_all, "OddsRatio_diseases_UKB.txt", row.names=F, sep="\t", quote=F)
