...
def prepare_data(data, target_col, covariates, seed = 42):
    ...
    print(X_test.loc[:, trans_covariates].dtypes)
    X_train.loc[:, trans_covariates] = scaler.fit_transform(X_train[trans_covariates])
    X_test.loc[:, trans_covariates] = scaler.transform(X_test.loc[:, trans_covariates])
...
def train_evaluate_model(X_train, y_train, X_test, y_test, input_config = 'GenMetS', encoding_method='normalize'):
...
if __name__ == '__main__':
    ...
    if not os.path.exists('./XGboost_100_three_models'):
        os.makedirs('./XGboost_100_three_models')
    for encoding_method in ['dummy', 'normalize']:
        if not os.path.exists(f'./XGboost_100_three_models/{encoding_method}'):
            os.makedirs(f'./XGboost_100_three_models/{encoding_method}')
    model_cofig = {
