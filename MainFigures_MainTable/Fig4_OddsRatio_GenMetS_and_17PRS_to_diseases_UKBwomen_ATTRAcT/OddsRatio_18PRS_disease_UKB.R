...
myresult_raw<- fread("OddsRatio_18PRS_to_diseases_UKBwomen.txt") %>% data.frame()
...
idx<- which(myresult_raw$disease=="Heart Failure")
myresult_raw$disease[idx]<- "HF"

idx<- which(myresult_raw$disease=="Hypertension")
myresult_raw$disease[idx]<- "HTN"
...
myresult_raw$disease <- factor(myresult_raw$disease,
                               levels=c("T2DM", "CAD", "HF",
                                            "HTN", "Stroke", "NAFLD"))

#F8766D
mycolors<- c(rep("black", 17), "#F8766D")
...
ggsave("OddsRatio_18PRS_in_UKB_disease_case_control.tiff", 
       width =8, height =6, units = "in",dpi = 300, compression = "lzw")

ggsave("OddsRatio_18PRS_in_UKB_disease_case_control.pdf", 
